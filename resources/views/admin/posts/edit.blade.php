...
<div class="card ">
    <div class="card-header d-flex align-items-center">
      ...
      <div class="card-body">
        <form class="row g-3" enctype="multipart/form-data" action="{{ route('posts.update', $posts->id) }}" method="POST" id="posts">
            @csrf
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" id="input-title" placeholder="Judul Berita" value="{{ old('title', $posts->title) }}" required>
                @error('title')
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="penulis" class="form-control @error('penulis') is-invalid @enderror" id="input-penulis" placeholder="Judul Berita" value="{{ old('penulis', $posts->penulis) }}" required>
                @error('penulis')
                <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
...
<script>
    $(document).ready(function() {
        $('#posts').submit(function(e) {
            ...
            $.ajax({
                url: '{{ route('posts.update', ':id') }}'.replace(':id', id), // Gunakan route update dan isi ID post yang akan diupdate
                type: 'POST',
                dataType: 'JSON',
                data: formData,
                cache: false,
