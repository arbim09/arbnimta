...
<div class="card ">
    ...
    <div class="card-body">
    <form class="row g-3" enctype="multipart/form-data" action="{{ route('events.update', $event->id) }}" method="POST" id="events">
        @csrf
        @method('PUT')
...
<script>
    $(document).ready(function() {
        $('#events').submit(function(e) {
            ...
            $.ajax({
                url: '{{ route('events.update', ':id') }}'.replace(':id', id), // Gunakan route update dan isi ID post yang akan diupdate
                type: 'POST',
                dataType: 'JSON',
                data: formData,
                cache: false,
